...
class vs_maker:
    def __init__(self, vertex_format='V', three_dimensional=True, auto_normal=False):
    ...
    def init_data(self):
        ...
        if 'N' in self.vertex_format:
            ...
            self.data += 'aPos;\n' if self.auto_normal else 'aNormal;\n'
        else:
            self.data += '\tFragPos = vec3(0);\n'
            self.data += '\tNormal = mat3(transpose(inverse(model))) * aPos;\n' if self.auto_normal else \
                '\tNormal = vec3(0);\n'
        self.data += '\tPos = aPos;\n}'
...
class GLSL_maker:
    ...
    light_fs: str = None
    no_light_sphere_fs: str = None
    light_sphere_fs: str = None
    ...
    def init_data():
        ...
        try:
            with open(os.path.join(fs_path, 'light_fs.fs'), 'r') as f:
                GLSL_maker.light_fs = f.read().replace('___set_max_dir_light_number',
                                                       str(GLSL_maker.set_max_dir_light_number)).replace(
                    '___set_max_point_light_number', str(GLSL_maker.set_max_point_light_number)).replace(
                    '___set_max_spot_lights',
                    str(GLSL_maker.set_max_spot_lights))
        except FileNotFoundError:
            raise FileNotFoundError("fs Shader file: light_fs.fs open error!")

        try:
            with open(os.path.join(fs_path, 'no_light_sphere_fs.fs'), 'r') as f:
                GLSL_maker.no_light_sphere_fs = f.read()
        except FileNotFoundError:
            raise FileNotFoundError("fs Shader file: light_fs.fs open error!")
        try:
            with open(os.path.join(fs_path, 'light_sphere_fs.fs'), 'r') as f:
                GLSL_maker.light_sphere_fs = f.read().replace('___set_max_dir_light_number',
                                                              str(GLSL_maker.set_max_dir_light_number)).replace(
                    '___set_max_point_light_number', str(GLSL_maker.set_max_point_light_number)).replace(
                    '___set_max_spot_lights',
                    str(GLSL_maker.set_max_spot_lights))
    ...
    @staticmethod
    def get_sphere_fs(use_light: bool = True, set_texture_number: int = 14):
        set_texture_number = clamp(set_texture_number, 0, GLSL_maker.set_max_texture_number)
        if GLSL_maker.light_sphere_fs is None:
            GLSL_maker.init_data()
        if use_light:
            return GLSL_maker.light_sphere_fs.replace('___set_texture_number', str(set_texture_number))
        else:
            return GLSL_maker.no_light_sphere_fs.replace('___set_texture_number', str(set_texture_number))

    @staticmethod
    def get_vs(vertex_format: str = 'V', three_dimensional=True, auto_normal=False):
        return vs_maker(vertex_format=vertex_format, three_dimensional=three_dimensional, auto_normal=auto_normal).data
